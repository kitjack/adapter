...
export default function RiddlesAnswerGenerator() {
  const [riddle, setRiddle] = useState('');
  const [difficulty, setDifficulty] = useState('medium');
  const [type, setType] = useState('wordplay');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/solve-riddle', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to solve riddle');
      }
      ...
      setError('Failed to solve riddle. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = () => {
    const content = solution.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'riddle-solution.txt';
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Solve Your Riddle</h2>
              <p className="text-gray-600">Get detailed explanations and step-by-step solutions</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="riddle" className="block text-sm text-gray-700 mb-2">
                  Enter Your Riddle
                </label>
                <textarea
                  id="riddle"
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition"
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 rounded text-cyan-500 focus:ring-cyan-500/20"
                  />
                  <span className="text-gray-700">Include step-by-step hints</span>
                </label>
              </div>
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-cyan-500 to-cyan-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <span>Solving riddle...</span>
                  </div>
                ) : cooldown > 0 ? (
                  `Wait ${cooldown}s to solve again`
                ) : (
                  'Solve Riddle'
                )}
              </button>
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {solution.map((step, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{step}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-cyan-500 group-hover:bg-cyan-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-cyan-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
