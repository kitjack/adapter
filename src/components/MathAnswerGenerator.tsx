...
export default function MathAnswerGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-math-answer', {
        method: 'POST',
        headers: {
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Solve Math Problems</h2>
              <p className="text-gray-600">Get step-by-step solutions with detailed explanations</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="problem" className="block text-sm text-gray-700 mb-2">
                  Math Problem
                </label>
                <textarea
                  id="problem"
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label htmlFor="detailLevel" className="block text-sm text-gray-700 mb-2">
                    Detail Level
                  </label>
                  <select
                    id="detailLevel"
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 rounded text-green-500 focus:ring-green-500/20"
                  />
                  <span className="text-gray-700">Show step-by-step work</span>
                </label>
              </div>
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-green-500 group-hover:bg-green-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-green-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
