...
export default function BlogFAQGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-blog-faq', {
        method: 'POST',
        headers: {
  ...
  const handleDownload = () => {
    const content = faqs.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `blog-faqs.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(faqs.join('\n\n'));
      setCopied(true);
    } catch (err) {
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Blog FAQs</h2>
              <p className="text-gray-600">Enter your topic and preferences to create comprehensive FAQ sections</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label htmlFor="numberOfQuestions" className="block text-sm text-gray-700 mb-2">
                    Number of Questions
                  </label>
                  <select
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {faqs.map((faq, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{faq}</div>
                    </div>
                  ))}
                </div>
              </div>
