...
export default function ScholarshipLetterGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to generate scholarship letter. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `scholarship-letter-${scholarshipName}.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(letter.join('\n\n'));
      setCopied(true);
    } catch (err) {
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Scholarship Letter</h2>
              <p className="text-gray-600">Create a compelling scholarship application letter</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="scholarshipName" className="block text-sm text-gray-700 mb-2">
                  Scholarship Name
                </label>
                <input
                  type="text"
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition"
              ...
              <div>
                <label htmlFor="academicInfo" className="block text-sm text-gray-700 mb-2">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition"
                  placeholder="Current education, GPA, major, relevant coursework"
                  rows={3}
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="achievements" className="block text-sm text-gray-700 mb-2">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition"
                  placeholder="Academic achievements, extracurricular activities, awards"
                  rows={3}
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition"
                  placeholder="Your academic and career aspirations"
                  rows={3}
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="financialNeed" className="block text-sm text-gray-700 mb-2">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition"
                  placeholder="Brief description of your financial circumstances"
                  rows={2}
                />
              </div>
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-indigo-500 to-indigo-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {letter.map((section, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{section}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-indigo-500 group-hover:bg-indigo-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-indigo-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
