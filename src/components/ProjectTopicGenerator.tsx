...
export default function ProjectTopicGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-project-topic', {
        method: 'POST',
        headers: {
  ...
  const handleDownload = () => {
    const content = topics.join('\n\n---\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `project-topics-${field}.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(topics.join('\n\n---\n\n'));
      setCopied(true);
    } catch (err) {
      console.error('Failed to copy topics:', err);
    }
  };
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Project Topics</h2>
              <p className="text-gray-600">Get innovative project ideas with detailed scope and objectives</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="field" className="block text-sm text-gray-700 mb-2">
                  Field/Domain
                </label>
                <input
                  type="text"
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition"
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-teal-500 to-teal-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {topics.map((topic, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{topic}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-teal-500 group-hover:bg-teal-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-teal-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
