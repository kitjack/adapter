...
export default function ListicleGenerator() {
  ...
  const [listicle, setListicle] = useState<string[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to generate listicle');
      }
      
      const data = await response.json();
      setListicle(data.listicle);
      setCooldown(15);
    } catch (err) {
      setError('Failed to generate listicle. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = () => {
    const content = listicle.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${topic.slice(0, 30)}-listicle.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(listicle.join('\n\n'));
      setCopied(true);
    } catch (err) {
      console.error('Failed to copy listicle:', err);
    }
  };
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Listicle</h2>
              <p className="text-gray-600">Create engaging list-style articles with customizable options</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-500/20 transition"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label htmlFor="numberOfItems" className="block text-sm text-gray-700 mb-2">
                    Number of Items
                  </label>
                  <select
                    id="numberOfItems"
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-fuchsia-500 to-fuchsia-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
            ...
            <div className="border-t border-gray-100">
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {listicle.map((item, index) => (
                    <div
                      key={index}
                      className="p-6 bg-gray-50 rounded-xl hover:bg-gray-100 transition cursor-pointer"
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{item}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-fuchsia-500 group-hover:bg-fuchsia-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-fuchsia-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
