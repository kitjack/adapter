...
export default function LiteratureReviewGenerator() {
  ...
  const handleDownload = () => {
    const content = review.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${topic.slice(0, 30)}-literature-review.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(review.join('\n\n'));
      setCopied(true);
    } catch (err) {
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Literature Review</h2>
              <p className="text-gray-600">Create comprehensive academic literature reviews</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition"
              ...
              <div>
                <label htmlFor="sources" className="block text-sm text-gray-700 mb-2">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition"
                  placeholder="Enter your sources (author, year, title, etc.)"
                  rows={5}
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label htmlFor="focus" className="block text-sm text-gray-700 mb-2">
                    Review Focus
                  </label>
                  <select
                    id="focus"
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-yellow-500 group-hover:bg-yellow-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-yellow-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
