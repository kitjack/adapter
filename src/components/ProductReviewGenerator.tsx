...
export default function ProductReviewGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-product-review', {
        method: 'POST',
        headers: {
  ...
  const handleDownload = () => {
    const content = reviews.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${productName.slice(0, 30)}-review.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(reviews.join('\n\n'));
      setCopied(true);
    } catch (err) {
      console.error('Failed to copy reviews:', err);
    }
  };
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Product Review</h2>
              <p className="text-gray-600">Enter product details to create balanced, informative reviews</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-lime-500 focus:ring-2 focus:ring-lime-500/20 transition"
              ...
              <div>
                <label htmlFor="productFeatures" className="block text-sm text-gray-700 mb-2">
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-lime-500 to-lime-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {reviews.map((review, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{review}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-lime-500 group-hover:bg-lime-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-lime-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
