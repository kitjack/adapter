...
export default function EssayOutlineGenerator() {
  ...
  const [mainPoints, setMainPoints] = useState('3');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-essay-outline', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ 
          topic,
          essayType,
          mainPoints: parseInt(mainPoints),
          style
        }),
      });
  ...
  const handleDownload = () => {
    const content = outline.join('\n\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${topic.slice(0, 30)}-outline.txt`;
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Essay Outline</h2>
              <p className="text-gray-600">Create structured essay outlines with detailed sections and points</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label htmlFor="essayType" className="block text-sm text-gray-700 mb-2">
                    Essay Type
                  </label>
                  <select
                    id="essayType"
                ...
                <div>
                  <label htmlFor="mainPoints" className="block text-sm text-gray-700 mb-2">
                    Main Points
                  </label>
                  <select
                    id="mainPoints"
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-amber-500 to-amber-600 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-amber-500 group-hover:bg-amber-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-amber-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
